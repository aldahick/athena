...
describe("context", () => {
  it("should register context generator for http", () => {
    ...
    class _HelloController {
      @get("/hello")
      hello(_req: HttpRequest, _res: HttpResponse, context: object): object {
    ...
    @contextGenerator()
    class _TestContextGenerator implements ContextGenerator {
      httpContext(_req: ContextRequest): Promise<object> {
  ...
  it("should register context generator for graphql", () => {
    ...
    class _HelloResolver {
      @resolveQuery()
      hello(_root: never, _args: never, context: object) {
        expect(context).toEqual({ test: "context" });
        return "hello, world!";
      }
    }
