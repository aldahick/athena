...
export class QueueRegistry {
  constructor(private readonly logger: LoggerService, private readonly redis: RedisService) {}

  async register(eventHandlerClasses: unknown[]): Promise<void> {
    const eventHandlers = eventHandlerClasses.map((c) => container.resolve<unknown>(c as InjectionToken)) as Record<string, unknown>[];
  ...
  private readonly onMessage =
    (eventName: string, callback: EventHandler) =>
    (message: string): void => {
